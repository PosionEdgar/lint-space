...
program
    .version(PKG_VERSION)
    .description(`${PKG_NAME} 是 前端编码规范工程化 的配套 Lint 工具，提供简单的 CLI 和 Node.js API，让项目能够一键接入、一键扫描、一键修复、一键升级，并为项目配置 git commit 卡点，降低项目实施规约的成本`);
...
program
    ...
    .option('-q, --quite', '仅报告错误信息 - 默认: false')
    .option('-o, --output-report', '输出扫描的规范问题日志')
    .option('-i, --include <dirpath>', '指定要进行规范扫描的目录')
    .option('--no-ignore', '忽略 eslint 的 ignore 配置文件盒 ignore 规则')
    .action(async (cmd) => {
        await installDepsIfThereNo();

        const checking = ora(`执行 ${PKG_NAME} 代码检查`);
...
program
    ...
    .option('-i, --include <dirpath>', '执行要进行修复的目录')
    .option('--no-ignore', '忽略 eslint 和 ignore 配置文件和 ignore 规则')
    .action(async (cmd) => {
        await installDepsIfThereNo();
