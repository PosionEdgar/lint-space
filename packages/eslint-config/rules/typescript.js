 ...
 * TypeScript ESLint 规则配置，用于 TypeScript 项目的代码规范检查
 * @link https://github.com/typescript-eslint/typescript-eslint
 */

// 导出 ESLint 配置对象
module.exports = {
    // 指定使用 TypeScript ESLint 解析器
    parser: '@typescript-eslint/parser',
    // 使用 TypeScript ESLint 插件
    plugins: ['@typescript-eslint'],
    // ESLint 插件设置
    settings: {
        // 为 TypeScript 文件应用特殊的解析规则
        'import/parsers': {
            '@typescript-eslint/parser': ['.ts', '.d.ts', '.tsx'], // 指定哪些文件扩展名使用 TypeScript 解析器
        },
        // 使用 eslint-import-resolver-typescript 来解析 TypeScript 模块
        'import/resolver': {
            typescript: {}, // 使用 TypeScript 模块解析器
        },
        // 在 import/extensions 设置中追加 TypeScript 文件扩展名
        'import/extensions': ['.js', '.ts', '.mjs'], // 支持的导入文件扩展名
    },
    // 解析器选项配置
    parserOptions: {
        project: './tsconfig.json', // 默认项目配置文件路径
        createDefaultProgram: true, // 兼容未在 tsconfig.json 中提供的文件
        extraFileExtensions: ['.vue'], // 额外的文件扩展名支持
    },
        rules: {
        /**
        ...
        // @typescript-eslint/no-loss-of-precision 需要 eslint 版本 >= v7，暂不启用
         ...
         * 防止在接口中错误地定义构造函数，或在类中错误地定义 new 方法
         */
        '@typescript-eslint/no-misused-new': 'off',
    
        /**
         * 【关闭】避免错误的使用 Promise
         * 防止在非异步函数中返回 Promise，或在条件表达式中错误地使用 Promise
         */
         ...
         * 防止在构造函数参数上使用 public、private、protected 等修饰符，推荐在类中显式声明属性
         */
         ...
         * 推荐抛出 Error 对象而不是字面量，以便更好地进行错误处理和调试
         */
        '@typescript-eslint/no-throw-literal': 'off',
    
        /**
         * 【关闭】禁止使用类型别名
         * 允许使用 type 关键字定义类型别名，这在某些场景下很有用
         */
    ...
    overrides: [
        {
            // 指定应用此配置的文件类型：TypeScript 和 TypeScript React 文件
            files: ['*.ts', '*.tsx'],
            // 针对 TypeScript 文件的特殊规则配置
            rules: {
                // 在 TypeScript 文件中禁用 no-undef 规则，因为它在导出默认接口时会错误地报错
                // TypeScript 编译器会在启用 strictNullChecks（或 strict）时捕获这些错误
                // 详见：https://github.com/iamturns/eslint-config-airbnb-typescript/issues/50
                'no-undef': 'off',
        
                /* 使用 TypeScript 使得禁用以下检查变得安全 */
        
                // 禁用基于 ESLint 的模块解析检查，以改善 monorepo 支持
                // TypeScript 编译器会处理模块解析，ESLint 的检查可能会与 TypeScript 的解析冲突
                // 详见：https://github.com/benmosher/eslint-plugin-import/blob/master/docs/rules/no-unresolved.md
